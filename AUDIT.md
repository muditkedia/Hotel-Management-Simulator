...
## 23. Chain HQ depth + long-term progression (`v31`, this branch — spec items #3 & #4)

**HQ (2+ hotels).** *Internal benchmarking*: every property ranked by a composite score with 30-day occupancy, RevPAR, GOPPAR and rating side by side — leader crowned, laggard flagged. *Inter-property staff transfers*: send the junior of any role from the active hotel to another (the A-team stays; the receiving roster tops up automatically; a hotel is never stripped of its last person in a role). *Capital allocation*: a monthly budget per property that the regional office invests while a regional manager is appointed — worst room conditions first, then stock — so remote hotels stop decaying. Shared marketing, training, purchasing, policies, the chain loyalty programme and the IPO layer already existed and are surfaced, not duplicated.

**Progression.** *Brand Prestige* (0–100) — the average of nine measurable pillars, each fed by an existing system: guest rating, national reputation, brand awareness, awards/certifications (incl. green & luxury), corporate accounts, loyalty membership, employee satisfaction (morale), local market share, and estate scale. A five-tier ladder (Local Guesthouse → City Player → Regional Brand → National Chain → Hospitality Icon) with the gap-to-next always visible on the **Goals** page, alongside **chain valuation** (assets + 6× annualised profit − debt) and **IPO readiness** (live count of listing requirements met). *Franchise programme*: at prestige 65+ with 2+ owned hotels, license the brand — capex-scaled upfront fee plus monthly royalties per unit; franchisees walk if chain reputation slides below 45, so the badge must be defended.

**Verified.** 15-point suite: prestige/tier math, Goals card render (single-hotel too), HQ benchmarking/allocation/transfer/franchise render, transfer semantics + last-person guard, regional-office capital investment (worst rooms first, cash spent), franchise prestige gate, sale, and isolated royalty flow (exact ₹120k for 2 units, revenue+cash together); 3-country sweep 0 issues; smoke4/5 and v27 suites green.

---

## 24. Reviews, staffing automation, loyalty & CCTV depth (`v32`, this branch)

**1 · Reviews.** The 240-review storage cap is gone — history is unbounded; only the UI list slices (latest 150). Review text is now journey-driven and voiced: guest-type lead-ins (business/family/VIP/wedding…, seeded per guest so runs differ), and specific mentions of what actually happened — an in-room fault and whether the duty technician fixed it, loyalty-point redemption and whether it felt worthwhile, the member welcome gesture, smooth early check-ins / late checkouts, room category living up to its billing, booking channel, and the deliberately different "blissfully uneventful" five-star. Consecutive reviews never share an opening. Verified: 12/12 unique bodies for identical quiet five-star stays.

**2 · Manager-managed hiring.** Per-hotel toggle (Staff → Hiring Desk) plus a per-department **seniority preference** (Junior/Experienced/Senior) that guides whoever hires. Each morning the GM computes staffing targets from the real weekly need (which already reflects occupancy, facilities and service complexity), then hires **breadth-first** — every short department gets one hire before any gets a second (≤6/day, 4 months' salary kept in reserve) — so F&B is never starved behind other departments. New facilities pull hiring automatically; new hires' rosters top up; manager-managed rosters refresh after hiring.

**3 · Corporate account load.** One manager now carries up to **five** accounts (was one); unassigned accounts are picked up automatically each month, largest first, while capacity allows; AM load (x/y) is visible on the Corporate page.

**4 · CCTV depth (still one frame per hour — timelapse feel preserved).** Lobby overlays are now *real*: waiting = actual unserved arrivals whose hour has passed; check-ins active = queue being served at the staffed desk; check-outs due from actual departure schedules; a Desk chip reads "N on duty" or "UNMANNED". Facility cameras show a diagonal **CLOSED** stamp outside their operating window, an **UNSTAFFED** banner when open without staff, and an Hours chip reflecting the configured schedule. Night frames dim with a NIGHT badge; shift-change figures cross at 06/14/22; a luggage cluster forms at the 10–12 checkout peak; busy public scenes gain a counter-drifting walker.

**5 · Facility operating hours.** Every service facility gets a schedule (house hours / custom window incl. overnight wrap / open-all-night / offline) on its management page. Outside trade scales with the open fraction of the default window; offline facilities drop to 30% running cost and stamp CLOSED on camera. In-house guests keep their own routines.

**6 · Loyalty & repeat business.** Redemption threshold 400→250 points (redemptions flagged into reviews); repeat-guest likelihood raised and now also scales with rating; returning-guest selection prefers guests who left satisfied (avgSat ≥65 favoured 70/30); members get a welcome gesture at check-in funded by the generosity budget; loyalty mentions in reviews are more frequent.

**7 · Service generosity budget.** The old percentage slider is now a **daily money budget** (Policies): complaint recovery at the desk (capex-scaled cost per save), member welcome touches — all drawn from today's allowance, which resets at midnight and reports yesterday's usage. Budget exhausted ⇒ recovery stops until tomorrow.
